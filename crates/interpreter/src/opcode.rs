...
pub const fn instruction<H: Host + ?Sized, SPEC: Spec>(
    opcode: u8,
) -> crate::tables::Instruction<H> {
    let table = instruction_table::<H, SPEC>();
    table[opcode as usize]
}

pub const fn instruction_table<H: Host + ?Sized, SPEC: Spec>(
) -> [crate::tables::Instruction<H>; 256] {
    use bytecode::opcode::*;
    let mut table = [control::unknown as crate::tables::Instruction<H>; 256];
